...
- `client` -> `clients`
- `employee` -> `employees`
- `pack_transport` -> `pack_transport_options`
...
docker exec -it work-order-v2-db mysql -uworkorder_v2 -p123456 -D work_order_v2 -e "SELECT COUNT(*) AS clients FROM clients; SELECT COUNT(*) AS employees FROM employees; SELECT COUNT(*) AS pack_transport_options FROM pack_transport_options; SELECT option_group, COUNT(*) AS cnt FROM option_items GROUP BY option_group ORDER BY option_group;"
```
...
### `work_orders`
新工單主表，先保留基礎欄位與 `client_id` / `pack_transport_option_id` 關聯。
