...
# 必須在載入 PyQt 前設定；此版本固定 1x 縮放避免 macOS Retina 擠版
...
class Main(QMainWindow, ui.Ui_Form):
    def __init__(self):
        ...
        # 針對 macOS 固定座標 UI：主動設定 splitter 比例，避免中間區塊擠在一起
        self._apply_layout_fixes()
        QtCore.QTimer.singleShot(0, self._apply_layout_fixes)
    ...
    def _set_widget_width(self, name, width):
        w = getattr(self, name, None)
        if w is not None:
            w.setMinimumWidth(width)
            w.setMaximumWidth(width)

    def _apply_hardfix_column_widths(self):
        # 強制中間主表格欄寬（固定 1x 下直接生效）
        # 目標欄寬：產品 300、材質 220、製程 260、備註 280
        for i in range(1, 16):
            self._set_widget_width(f'comboBox_product_{i}', 300)
            self._set_widget_width(f'comboBox_material_{i}', 220)
            self._set_widget_width(f'comboBox_process_{i}', 260)
            self._set_widget_width(f'comboBox_plate_{i}', 130)
            self._set_widget_width(f'comboBox_thicknes_{i}', 130)
            self._set_widget_width(f'comboBox_others_{i}', 280)
            self._set_widget_width(f'lineEdit_width_{i}', 110)
            self._set_widget_width(f'lineEdit_height_{i}', 110)
            self._set_widget_width(f'lineEdit_amount_{i}', 100)
            self._set_widget_width(f'lineEdit_CBMprice_{i}', 115)
            self._set_widget_width(f'lineEdit_CBM_{i}', 115)
            self._set_widget_width(f'lineEdit_single_price_{i}', 150)

        for i in range(16, 31):
            self._set_widget_width(f'lineEdit_single_price_{i}', 150)

    def _apply_layout_fixes(self):
        self.setMinimumSize(2100, 980)

        # splitter_4: 左邊人員下拉 + 右邊數值欄
        if hasattr(self, 'splitter_4'):
            self.splitter_4.setChildrenCollapsible(False)
            self.splitter_4.setStretchFactor(0, 1)
            self.splitter_4.setStretchFactor(1, 1)
            self.splitter_4.setSizes([520, 520])

        # splitter: 左邊 splitter_4 + 右邊備註區
        if hasattr(self, 'splitter'):
            self.splitter.setChildrenCollapsible(False)
            self.splitter.setStretchFactor(0, 2)
            self.splitter.setStretchFactor(1, 3)
            self.splitter.setSizes([1200, 1700])

        # splitter_2: 上方人員區 + 下方其他區塊
        if hasattr(self, 'splitter_2'):
            self.splitter_2.setChildrenCollapsible(False)
            self.splitter_2.setStretchFactor(0, 3)
            self.splitter_2.setStretchFactor(1, 2)
            self.splitter_2.setSizes([500, 320])

        self._apply_hardfix_column_widths()
...
if __name__ == '__main__':
    ...
    # macOS：固定 1x，強制關閉 High DPI 自動縮放
    if sys.platform == 'darwin':
    ...
    # 不使用 showMaximized（在 retina 下容易造成固定座標 UI 跑版）
    window.resize(2300, 1050)
    window.setMinimumSize(2100, 980)
    window.show()
